...
export const findOneWithRelationsSeparateQueries = async (
    ...
    try {
        ...
        const mainResult = await (pool as pg.Pool).query(mainQuery, [primaryKeyValue]);
        ...
        for (const relation of relations) {
            ...
            const relatedResult = await (pool as pg.Pool).query(relatedQuery, [primaryKeyValue]);
            // If it's a one-to-one relationship, return single object
            // If it's one-to-many, return array
            result[relation.tableName] = relatedResult.rows.length === 1 
