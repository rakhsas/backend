...
export const areFriends = async (req: any, res: Response) => {
	try {
		const userId: string = req.userId;
		// get the other user id from the param

		const likedId: string = req.params.id;
		const areFriends = await likeService.areFriends(userId, likedId);
		res.status(200).json({ status: areFriends });
	} catch (err: any) {
		res.status(HttpStatus.BAD_REQUEST).json({
			error: err.message,
		});
