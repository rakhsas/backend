...
export const areFriends = async (user1Id: string, user2Id: string) => {
	try {
		const query = `
			SELECT * FROM likes
				WHERE (
					(user_id = $1 AND liked_id = $2)
					OR (user_id = $2 AND liked_id = $1)
				)
				AND NOT EXISTS (
		...
		// console.log('Are friends query result:', query, [user1Id, user2Id]);
