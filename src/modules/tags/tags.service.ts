...
export const saveUserTags = async (body: any) => {
	...
	try {
		const allTagsHaveOnlyValue = tags.every(tag => {
			// Check if tag has only value and no other properties
			const keys = Object.keys(tag);
			console.log('Tag keys:', keys);
			return keys[0] === 'value' && tag.value !== '';
		});
		console.log('allTagsHaveOnlyValue:', allTagsHaveOnlyValue);
		if (allTagsHaveOnlyValue) {
			console.log('No actual changes to tags, skipping update');
			return { success: true, message: 'No tag changes detected' };
		}
		// we can simply check if tags entries have value to return;
...
export const getTagById = async (tagId: string) => {
	try {
		...
		`;
		const result = await repository.executeSqlQuery(query, [tagId]);
